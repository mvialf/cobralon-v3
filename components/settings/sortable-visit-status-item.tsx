...
interface SortableVisitStatusItemProps {
  status: VisitStatus
  isDraggable: boolean
...
export function SortableVisitStatusItem({
  status,
  isDraggable,
  onEdit,
  onDelete,
}: SortableVisitStatusItemProps) {
  ...
  return (
    ...
    >
      <div className="flex items-center gap-3">
        ...
        {status._count.visits > 0 && (
          <span className="text-muted-foreground text-xs">
            ({status._count.visits} visita{status._count.visits !== 1 ? 's' : ''})
          </span>
        )}
      </div>
