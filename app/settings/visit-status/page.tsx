...
import { SortableVisitStatusItem } from '@/components/settings/sortable-visit-status-item'
import type { BadgeColor, VisitStatus } from '@/lib/validations/visit-status-validations'

export default function VisitStatusSettingsPage() {
  ...
  const fetchStatuses = async () => {
    ...
    setStatuses(data.visitStatuses)
  }
  ...
  const handleDelete = async () => {
    ...
    try {
      const response = await fetch(`/api/visit-status/${selectedStatus.id}`, {
        method: 'DELETE',
      })
  ...
  const openEditDialog = (status: VisitStatus) => {
    setSelectedStatus(status)
    setIsEditDialogOpen(true)
  }

  const openDeleteDialog = (status: VisitStatus) => {
    setSelectedStatus(status)
  ...
  const handleDragEnd = async (event: DragEndEvent) => {
    ...
    try {
      ...
      const response = await fetch('/api/visit-status/reorder', {
        method: 'POST',
      ...
      <Card>
        <CardHeader>
          <div className="flex items-center justify-between">
            <CardTitle>Estados de Visita</CardTitle>
        ...
        <CardContent>
          ...
          <div className="mt-6 rounded-lg border bg-muted/40 p-4">
            <p className="text-muted-foreground text-sm">
              ...
              editar visitas. Arrastra los estados normales para reordenarlos. El estado inicial y
        ...
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              ...
              {selectedStatus && selectedStatus._count.visits > 0 && (
                <span className="mt-2 block font-semibold text-destructive">
                  Atención: Este estado tiene {selectedStatus._count.visits} visita(s) asignada(s) y
          ...
          <AlertDialogFooter>
            ...
            <AlertDialogAction
              ...
              disabled={selectedStatus ? selectedStatus._count.visits > 0 : false}
            >
              Eliminar
            </AlertDialogAction>
          </AlertDialogFooter>
