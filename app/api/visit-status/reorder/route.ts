...
export async function POST(request: Request) {
  try {
    ...
    const existingStatuses = await prisma.visitStatus.findMany({
      where: {
        id: { in: statusIds },
      },
      select: {
    ...
    const allStatuses = await prisma.visitStatus.findMany({
      where: { isActive: true },
      orderBy: { order: 'asc' },
      include: {
        color: {
    ...
    console.error('Error reordering visit statuses:', error)
    return NextResponse.json({ error: 'Error al reordenar los estados de visita' }, { status: 500 })
  }
}
