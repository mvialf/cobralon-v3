...
```typescript
// tests/e2e/pages/customer-page.ts
import { Page, Locator, expect } from '@playwright/test'
...
export class CustomerPage extends BasePage {
  ...
  async search(term: string) {
    const responsePromise = this.page.waitForResponse(
      r => r.url().includes('/api/customer') && r.ok()
    )
...
```typescript
// tests/e2e/pages/customer-form.ts
import { Page, Locator, expect } from '@playwright/test'
...
export class CustomerForm {
  ...
  /** Submit y esperar respuesta del servidor */
  async submit() {
    const responsePromise = this.page.waitForResponse(
  ...
  /** Submit y esperar que cierre el dialog (éxito) */
  async submitAndExpectSuccess() {
    await this.submit()
    await expect(this.dialog).not.toBeVisible({ timeout: 15000 })
  }

  /** Submit y esperar que el dialog permanezca abierto (error) */
...
test('crear cliente', async ({ page }) => {
  ...
  const dialog = await customerPage.openNewDialog()
  const form = new CustomerForm(page)
...
test('maneja error de email duplicado', async ({ page }) => {
  ...
  await form.submitAndExpectError()
})
```
