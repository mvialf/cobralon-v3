 ...
 * Adaptado de useUninstallTags para integrantes del equipo
 */

import { useState, useCallback, useEffect } from 'react'
import type { TagColor } from '@/components/custom/tag-system/types'

// Tipo TeamTag compatible con el sistema de tags
  ...
  const createTag = useCallback(
    async (name: string, abbreviation: string, colorId: string) => {
      try {
        ...
        const errorMessage = err instanceof Error ? err.message : 'Error al crear integrante'
        setError(errorMessage)
        throw err
      }
    },
  ...
  const editTag = useCallback(
    async (tagId: string, name: string, abbreviation: string, colorId: string) => {
      try {
        ...
        if (!res.ok) {
          const data = await res.json()
          throw new Error(data.error || 'Error al editar integrante')
        }

        await refreshTags()
      } catch (err) {
        const errorMessage = err instanceof Error ? err.message : 'Error al editar integrante'
        setError(errorMessage)
        throw err
      }
    },
  ...
  const deleteTag = useCallback(
    async (tagId: string) => {
      try {
        ...
        const errorMessage = err instanceof Error ? err.message : 'Error al eliminar integrante'
        setError(errorMessage)
        throw err
      }
    },
