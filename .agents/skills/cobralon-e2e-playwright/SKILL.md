...
description: Usar para escribir, refactorizar o depurar tests E2E de Cobralon con Playwright en tests/e2e, page objects, auth setup, cleanup, waitForResponse, selectores accesibles, datos determinísticos o flujos de UI críticos. Complementa cobralon-testing-strategy.
---
...
│   └── test-data-factory.ts   # Datos E2E reutilizables
...
const responsePromise = page.waitForResponse(
  resp => resp.url().includes('/api/customers') && resp.ok()
)
...
const searchResponse = page.waitForResponse(r => r.url().includes('/api/customers'))
