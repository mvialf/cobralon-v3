...
description: Usar para errores de Cobralon en API routes, withApiHandler, BusinessError, ApiError, createApiError, handleMutationError, Pino logging, toast notifications, error boundaries app/error.tsx o app/global-error.tsx, y respuestas HTTP estandarizadas.
---
...
El proyecto tiene boundaries globales en `app/error.tsx` y `app/global-error.tsx`. Para errores esperados de dominio, seguir usando:
- `withApiHandler` y `BusinessError` en API routes
...
|---------|-------|-------------|
| `docs/rules/api-routes.md` | Estructura base de routes | Error handling básico |
