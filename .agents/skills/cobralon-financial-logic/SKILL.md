...
description: Usar al tocar pagos, PaymentAllocation, FIFO, ProjectFinancials, creditBalance, CreditTransaction, refunds, ajustes, balances, deudas o cualquier archivo en lib/business-logic/ relacionado con dinero. Cubre invariantes financieros de Cobralon, módulos reales, transacciones Prisma y tests no-watch.
---
...
| `project-financials.ts` | Lectura de `ProjectFinancials` y balance derivado |
| `credit-management.ts` | CreditBalance desde ledger y helpers de crédito |
| `credit-rules.ts` | Distribución pago/crédito, validación |
...
| `money.ts` | Operaciones Decimal seguras, comparaciones y redondeo |
...
- **API routes** (`docs/rules/api-routes.md`): transacciones, validación Zod en endpoints financieros
- **Database** (`docs/rules/database.md`): comandos Prisma, reglas FIFO, transacciones
