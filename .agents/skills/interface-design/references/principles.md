...
The project's `globals.css` already defines the primitives. Read and use them:

- **Foreground** — `foreground`, `muted-foreground`, `card-foreground`, plus opacity modifiers
- **Background** — `background`, `card`, `popover`, `sidebar`, `muted`, `accent`, `secondary`
- **Border** — `border`, `input`, `ring`
- **Brand** — `primary` (and its foreground)
- **Semantic** — `destructive`, `success` (and their foregrounds)

Don't invent new colors. Map everything to these existing tokens.
...
The project already defines control-specific tokens:

- **Control border** — `--input` token, mapped via `border-input`
- **Control focus** — `--ring` token, mapped via `ring-ring`
- **Control surfaces** — inputs use `bg-transparent` or `bg-background` depending on context

Use these instead of reusing surface tokens for interactive elements.
...
Use the project's existing depth tokens — don't define new shadow values:

```
/* Borders-only — use the project's border token */
border-border          /* standard separation */
border-border/50       /* softer separation via opacity */

/* Shadow scale — use the project's shadow tokens */
shadow-2xs → shadow-xs → shadow-sm → shadow → shadow-md → shadow-lg → shadow-xl → shadow-2xl

/* Example Tailwind classes */
border shadow-sm       /* subtle card depth */
border shadow-none     /* flat, borders-only */
shadow-md              /* floating elements like dropdowns */
```
