...
**Before any design work, read the project's token file** — typically `app/globals.css` or equivalent.

This is non-negotiable. The project already has a design system: colors, spacing, shadows, radius, fonts. Your job is to design WITHIN that system, not alongside it.

## The Rule

**Every visual value in your output must trace to an existing token.** No raw hex, oklch, rgba, or px values inline. Use Tailwind classes that reference the project's CSS variables.

```
✅ bg-sidebar text-sidebar-foreground
✅ bg-primary text-primary-foreground
✅ border-border shadow-sm rounded-lg
✅ text-muted-foreground

❌ bg-[#1a2332] text-[oklch(0.88_0.007_248)]
...
If your design requires something the token system doesn't provide:
1. Name what's missing and WHY
2. Propose an addition to `globals.css` as an explicit change
3. Get user approval before adding
4. NEVER work around it with inline values

## Token Inventory

After reading `globals.css`, produce a mental map of available tools:

- **Surfaces:** background, card, popover, sidebar, muted, accent, secondary
- **Text:** foreground, card-foreground, muted-foreground, primary-foreground, etc.
- **Borders:** border, input, ring
- **Semantic:** primary, destructive, success
- **Depth:** shadow-2xs through shadow-2xl
- **Radius:** radius-sm through radius-xl
- **Spacing:** the base --spacing unit and its multiples

This inventory IS your palette. Design with it.
...
**Token names feel like given constraints.** The project's CSS variables are decisions already made. `--sidebar`, `--primary`, `--muted-foreground` — these define what's available. Your job isn't to rename or reinvent them. It's to use them with intention, choosing WHICH token for WHICH purpose. The creative act is in the mapping, not the naming.
...
**Token mapping:** Read `globals.css`. Map the existing tokens to the intent. Which surface tokens carry the identity? Which semantic tokens communicate status? Which combinations create the right feel? The palette is fixed — the craft is in how you combine what exists.

**Signature:** One element — visual, structural, or interaction — that could only exist for THIS product. Must be achievable with existing tokens. If you can't name one, keep exploring.
...
- Specific token names from `globals.css` and how you'll use them
- Your signature element
- What replaces each default

**The test:** Read your proposal. Does every color/surface/shadow reference an actual token from the project? If you see raw values, you're inventing instead of designing.
...
- **The token test:** Does every visual value in your output trace to a token from `globals.css`? Search for raw hex, oklch, rgba, or arbitrary Tailwind values (`[...]`). If you find any, replace them with token references.
...
## Color Already Lives in globals.css

The project's palette is defined. Don't invent new colors — understand the ones that exist and use them with purpose.

Read the token file and understand the system: which tokens are for surfaces, which for text hierarchy, which for interactive states, which are semantic. The creative work is in the COMBINATIONS and CONTEXT — using `bg-sidebar` for a brand panel, pairing `text-muted-foreground` with `text-foreground` for hierarchy, choosing `shadow-sm` over `shadow-md` for subtlety.

**Color Carries Meaning:** Gray builds structure. Color communicates — status, action, emphasis, identity. Unmotivated color is noise. One accent color (`primary`), used with intention, beats spreading semantic tokens without thought.

**Opacity as Expression:** You can't create new colors, but you CAN modulate existing ones. `text-sidebar-foreground/60` and `text-sidebar-foreground/40` create hierarchy from a single token. This is your expressive tool within the constraint.
...
Tokens: [which tokens from globals.css you'll use — and WHY these over others]
Depth: [which shadow/border tokens — and WHY this level]
Surfaces: [which surface tokens (bg-background, bg-card, bg-sidebar, etc.)]
Typography: [font-sans/font-mono from the project — no external fonts]
Spacing: [the project's --spacing base unit and which multiples]
```

This checkpoint is mandatory. Every value must trace to `globals.css`.

If you can't name the specific token for a choice, you're about to hardcode. Stop and find the right token.
...
The project's `globals.css` already defines the primitives. Read them, understand the hierarchy, use them consistently. No random hex values — everything maps to existing tokens.

### Text Hierarchy

Use the project's text tokens to build hierarchy: `text-foreground` (primary), `text-muted-foreground` (secondary/metadata), `text-primary` (accent), opacity modifiers like `text-foreground/60` for tertiary levels. If you're only using one or two levels, your hierarchy is too flat.

### Border Progression

Use the project's border and opacity system: `border-border` for standard separation, `border-border/50` for softer separation, `ring-ring` for focus emphasis. Match intensity to the importance of the boundary.

### Control Tokens

The project defines `--input` and `--ring` specifically for form controls. Use these — they're tuned independently from layout surfaces.
...
## Suggest + Ask
Lead with your token inventory and recommendation, then confirm:
```
"Tokens available: [key tokens from globals.css grouped by purpose]
Domain: [5+ concepts from the product's world]
Token mapping: [which tokens carry the identity, e.g. bg-sidebar for brand panels, text-primary for accents]
Signature: [one element unique to this product, achievable with existing tokens]
...
## Step 0: Always Read Tokens First
Before ANY design work, read `app/globals.css` (or the project's token file). This is mandatory regardless of whether system.md exists.

## If Project Has system.md
Read `.interface-design/system.md` and apply. Decisions are made. Still verify against current `globals.css`.

## If No system.md
1. **Read tokens** — `globals.css`, inventory available tokens
2. Explore domain — Produce all four required outputs (domain, token mapping, signature, defaults)
3. Propose — Direction must reference actual token names
4. Confirm — Get user buy-in
5. Build — Apply principles, every value traces to a token
6. **Evaluate** — Run the mandate checks (including token test) before showing
7. Offer to save
